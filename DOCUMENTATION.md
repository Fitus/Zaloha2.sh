...
For automatic operations, use the <b>--noExec</b> option to tell Zaloha to not
...
<b>unl.UP</b>    unlink multiply-linked file in &lt;backupDir&gt; + <b>UPDATE</b> (can be switched
          to just <b>UPDATE</b> via the <b>--noUnlink</b> option, see below)
<b>unl.UP.!</b>  unlink multiply-linked file in &lt;backupDir&gt; + <b>UPDATE.!</b> (can be switched
          to just <b>UPDATE.!</b> via the <b>--noUnlink</b> option, see below)
<b>unl.UP.?</b>  unlink multiply-linked file in &lt;backupDir&gt; + <b>UPDATE.?</b> (can be switched
          to just <b>UPDATE.?</b> via the <b>--noUnlink</b> option, see below)
...
-----------------------------------
<b>REV.MKDI</b>  reverse-create a directory in &lt;sourceDir&gt; due to <b>REV.NEW</b>
<b>REV.NEW</b>   reverse-create a file in &lt;sourceDir&gt; (if a standalone file in
          &lt;backupDir&gt; is newer than the last run of Zaloha (or in case
          of the <b>--revNewAll</b> option irrespective of it))
<b>REV.UP</b>    reverse-update file in &lt;sourceDir&gt; (if the file in &lt;backupDir&gt;
          is newer than the file in &lt;sourceDir&gt;)
<b>REV.UP.!</b>  <b>REV.UP</b> on a file in &lt;sourceDir&gt; which is newer than the
          last run of Zaloha (or newer than the last run of Zaloha
          minus 3600 secs if the option <b>--ok3600s</b> is given)

Exec4:  remaining removals from &lt;backupDir&gt;
        (can be switched off by the <b>--noRemove</b> option)
-----------------------------------
<b>RMDIR</b>     regular remove directory from &lt;backupDir&gt;
<b>REMOVE</b>    regular remove file from &lt;backupDir&gt;
<b>REMOVE.!</b>  remove file from &lt;backupDir&gt; which is newer than the
...
Exec5:  updates resulting from optional comparing of the contents of files
        (optional feature, can be activated via the <b>--byteByByte</b> or
         the <b>--sha256</b> options)
-----------------------------------
<b>UPDATE.b</b>  update file in &lt;backupDir&gt; because its contents is not identical
<b>unl.UP.b</b>  unlink multiply-linked file in &lt;backupDir&gt; + <b>UPDATE.b</b> (can be switched
          to just <b>UPDATE.b</b> via the <b>--noUnlink</b> option, see below)

(internal use, for completeness only)
-----------------------------------
<b>OK</b>        object without needed action or an object not to be synchronized.
...
removes (unlinks) them first (action code <b>unl.UP</b>), to prevent "updating" of
...
eventually missing but needed parent directories (<b>REV.MKDI</b>).
...
in &lt;sourceDir&gt; (action code <b>REV.UP</b>).
...
If the reverse-synchronization is not active: If neither <b>--revNew</b> nor
...
The action codes are <b>UPDATE.b</b> and <b>unl.UP.b</b> (the latter is an update with prior
...
<b>--revUp</b>         ... enable <b>REV.UP</b> (= if the file in &lt;backupDir&gt; is newer than
...
<b>--sha256</b>        ... compare contents of files via SHA-256 hashes. There is an
                    ...
                    modification times will get aligned (<b>ATTR:T</b>). This option
...
<b>--pMode</b>             and SLINK (here except the modes) actions. If none of these
                    ...
                    only if their synchronization is active (<b>--syncSLinks</b>).
...
<b>--optimCSV</b>      ... optimize space occupied by the CSV metadata files
...
<b>--noLastRun</b>     ... do not obtain time of the last run of Zaloha by running
                    ...
                    features based on the last run of Zaloha: <b>--revNew</b> and
...
in advance. Please note that if the option <b>--followSLinksS</b> is given, then
...
leading to <b>NEW</b> and <b>REMOVE</b> actions.
...
This mode is activated by the <b>--sourceUserHost</b> option.
...
To use a different port, use the options <b>--sshOptions</b> and <b>--scpOptions</b> too
...
To fix this, use <b>--scpOptions</b> to pass the "-O" compatibility option to SCP.
...
The first useful option is <b>--noDirChecks:</b> This switches off the checks for
existence of &lt;sourceDir&gt; and &lt;backupDir&gt;.

In the Local Mode, if &lt;backupDir&gt; is not available locally, then it is necessary
to use the <b>--metaDir</b> option to place the Zaloha metadata directory
to a location accessible to Zaloha.

Other useful options are <b>--noFindSource</b> and/or <b>--noFindBackup:</b> They instruct
...
The last set of useful options are <b>--no610Hdr</b> through <b>--no653Hdr.</b> They
